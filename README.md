...
## cstack — Autonomous Agent Fleet

cstack is the autonomous multi-agent loop built on top of gstack. Multiple Claude agents run in parallel, coordinate through a shared control repo, and collaborate in real-time using a kernel-enforced task ledger.
...
#### Step 2 — Edit `supervisor/fleet.conf` to match your agents
...
**Development** (one terminal, agents run in the background, stop when you close the window):
...
See [`supervisor/README.md`](supervisor/README.md) for real-time collaboration details, Supabase wake configuration, and adding new agents.
